...
class APK():
    ...
    def _extract_callers(self, api_called):
        ...
        for each_meth in api_called:
            for _, call, _ in each_meth._method_analysis_object.get_xref_from(
                ...
                try:
                    if each_meth not in callers[
                            caller.get_id()]['apis_called']:
                        callers[caller.get_id()]['apis_called'].append(
                            each_meth)
                except KeyError:
    ...
    def export(self):
        attribute_dictionary = dict([
            ...
            ("permissions_in_xml",
             [str(x) for x in self.get_permissions_in_xml()]),
...
class AbstractPrivacyMethod:
    ...
    def refine_class_name(method_name):
        ...
        if regex_match is not None:
            ...
            refined_method_name = refined_method_name[:
                                                      start] + refined_method_name[
                                                          end - 1]
    ...
    def export(self):
        attribute_dictionary = dict([("id", self.get_id()),
...
class PrivacyAPI(AbstractPrivacyMethod):
    ...
    def export(self):
        ...
        attr_dict = dict([("personal_information_collected",
                           self.get_personal_information_collected()),
                          ("permissions_required",
                           self.get_permissions_required()),
